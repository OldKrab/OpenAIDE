...
**Implementation:** The shared Frontend now depends on one installed `FrontendShell` contract with separate message and navigation capabilities. `startFrontend(shell)` is its only mounting interface. The browser composition root and standalone preview adapter live under `apps/browser`, select and install the concrete shell before React mounts, and are the only code that detects the hosting environment. Shared production code no longer detects VS Code, parses Web URLs, mutates browser history, or dispatches the `openaide:webRoute` DOM event. Web route parsing/history lives in `apps/web/frontend/webAppShell.ts`, and VS Code API/message routing lives in `apps/vscode-extension/frontend/vsCodeShell.ts`. Surface navigation uses focused `openNewTask`, `openSettings`, and `openTask` methods instead of sending navigation through the broad host-message channel. The old runtime-selecting `hostBridge` implementation and shared-package shell selector were removed completely, with no compatibility path.
...
**Implementation:** `TaskRecord` stores one `TaskTitleState`: an optional Prompt/Agent automatic title and, when selected, a User override. New Tasks leave the automatic title absent, while first Send stores a normalized 60-character Prompt prefix in the same commit that makes the Task visible. Agent metadata and reconciled `session/list` titles replace or clear the automatic title even while a User override is visible. `task/setTitle` can select a normalized User override or reset to the latest automatic title without consulting the Agent. Native Session adoption records its supplied title as Agent-owned. `"New task"` and `"Untitled task"` remain Frontend-only fallbacks when no visible stored title exists.
...
**Implementation:** `useAppController` is a composition root that wires the reducer, App Server lifecycle, retained `NewTaskController`, New Task workspace, routed Task workspace, and user-intent callbacks without implementing those workflows itself. Its UI contract now exposes a render-ready `view` projection and named New Task and Task intents; raw `AppState`, reducer `dispatch`, request generations, and retained New Task snapshots stay inside the controller. `NewTaskView` and `TaskView` consume those typed intents instead of constructing reducer actions. `useNewTaskWorkspace` owns preparation, attachment resources, retained selection, and Native Session discovery, while `useTaskWorkspace` owns visible Task projection, read receipts, empty-Task routing, and render telemetry. The App Server lifecycle delegates routed Task subscription, open readiness, failure, and retry to `useTaskRouteLifecycle`. Existing workflow modules were retained only where they already owned cohesive behavior, and old interfaces were removed rather than wrapped for compatibility.
...
History freshness must not compare Native Session activity with generic Task `updated_at` or with the timestamp of the last history synchronization. Generic Task metadata can change without Chat, while Chat can continue changing through live Agent work after a synchronization. Persist a dedicated `localHistoryUpdatedAt` that advances whenever stored Chat changes, and compare the cached Native Session `updatedAt` against that value when the user opens the Task or a complete catalog observation advances an idle, subscribed Task.

**Resolution:** First Send now commits and returns a `starting` Task without entering history synchronization. A deep `NativeSessionService` owns preparation, start/load/resume/recreate selection, session binding, one update subscription, and primary prompt startup; the Send workflow only hands it the accepted prompt. Task open consults a separately refreshed Native Session catalog and starts `session/load` only when its cached native timestamp is more than five seconds newer than the dedicated durable Chat clock. After a complete catalog observation, App Server runs the same freshness check for subscribed, idle Tasks so an already-open Task can refresh without reopening. Successful replay replaces Chat and advances that clock to the native timestamp, while failure records a Live Activity and returns history state to idle. The catalog refreshes when Task Navigation gains a subscriber, every five minutes while demand remains, and on explicit discovery requests; Send does not refresh it. The obsolete checking/failed history states and retry method were removed.
