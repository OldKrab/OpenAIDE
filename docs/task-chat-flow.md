...
- The Web App first streams each selected file or Image through one authenticated same-origin request directly into a unique App Server OS temp file. If that request receives HTTP 413, receives an HTML HTTP 403 from a request-size gateway, or fails at the transport boundary, the Web App automatically restarts that attachment from byte zero through an authenticated upload session using sequential binary chunks no larger than 512 KiB. App Server scopes the session to the initialized client, requires exact offsets and stable Task/file metadata, and creates a sendable attachment handle only after the declared byte count is complete. Completed Images are converted into the existing image-handle representation inside App Server, so their base64 data is never carried in the reliable-session JSON request. It runs at most two general-file uploads concurrently, renders per-file progress and cancellation, and blocks Send until every selected upload settles. A failed upload remains visible with a concise inline error, can be dismissed, and can be retried without reselecting while the browser still retains the selected file; after chunk fallback has begun, Retry starts directly in chunk mode. Known partial/failed files and chunk sessions are deleted or expired best-effort. Completed files remain under OS temp lifetime management. A sent-file click streams the still-available file back through an authenticated Task-and-message-scoped download route.
