...
An Open Task may be renamed through `task/setTitle`. Every accepted title is whitespace-normalized into one line and bounded at 200 characters; oversized automatic titles are truncated with an ellipsis, while oversized User titles are rejected. A User title is also non-empty. It becomes the visible title without discarding the automatic title. Agent title updates continue to advance the hidden automatic value while the override is visible. Resetting through the same method selects automatic ownership and immediately reveals the latest automatic title, or the normal untitled fallback if the Agent cleared it. Rename and reset update Task metadata but do not count as Task activity or reorder the Task list. Archived Tasks are immutable and reject either mutation.
