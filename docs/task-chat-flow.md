...
The primary prompt and every steering prompt accepted during its active Task turn form one current prompt set. The first current prompt response with `end_turn`, whether it belongs to the primary prompt or a steering prompt, ends that Task turn and publishes idle state. Prompt settlement and Send admission share one per-Task ordering boundary: a Send ordered after settlement starts a new Task turn even when its client has not yet observed idle, while steering already accepted before settlement remains part of the ended prompt set. Each remaining response waiter retains ownership of the ended Native Session prompt generation until its ACP request settles. A later Task turn may be accepted durably during that drain, but its primary ACP prompt does not dispatch until every request in the earlier generation has settled. Those eventual responses are stale and cannot finish, revive, or otherwise mutate the ended Task turn. Session updates remain owned by the session-lifetime update consumer and continue to be projected in arrival order.
...
The VS Code Extension may present a Task Attention Event through VS Code's workbench Notification Center while its extension host is active. An event is eligible when its Task was not the active Task editor at the event occurrence time, including while another Task, source file, or workbench surface was active. Losing VS Code window focus does not make the active Task eligible. Its first Task-navigation snapshot establishes a baseline and never turns existing attention into a notification backlog. Extension-global delivery receipts suppress replay across restart, and VS Code's own per-extension notification filter and Do Not Disturb controls provide local enablement. The notification contains only the short product reason and a single-line, compact form of the Task title; selecting its **Open Task** action routes to the Task without changing its complete title. VS Code extension notifications remain inside the VS Code workbench and are not macOS, Windows, or Linux system notifications.
