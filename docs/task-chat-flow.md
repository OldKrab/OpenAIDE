...
For high-confidence, read-only shell forms, App Server may persist a semantic presentation hint (`skill`, `read`, `list`, or `search`) on an `execute` Tool. The hint changes only its compact title, icon, and grouped summary. The Tool remains `execute`, and expanding it always preserves the complete execute command, output, status, permission history, and terminal details. Unsupported, mixed, ambiguous, or potentially effectful shell forms receive no hint.
