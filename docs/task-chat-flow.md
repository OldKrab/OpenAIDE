...
Only the Native Session Catalog calls `session/list`. It discovers all enabled Agents for every visible Project root and available worktree, coalesces overlapping refresh demand, publishes successful pages independently, and retains cursors only inside App Server. Discovery starts when Task Navigation gains a subscriber, repeats every five minutes while demand remains, and also responds to `taskNavigation/refresh` and `taskNavigation/loadMore`. Sending a message does not initiate discovery.
