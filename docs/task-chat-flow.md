...
App Server persists successful `session/list` observations and returns them in the immediate subscription baseline after restart. It never deletes a cached row merely because a partial or failed listing omitted it. A definitive not-found response from `session/load` removes the stale Native Session while its opening surface reports that it no longer exists. General missing-session reconciliation and its authoritative-evidence rules are specified below.

Only the Native Session Catalog module calls `session/list`. It discovers all enabled Agents for every visible Project root and available worktree, coalesces overlapping refresh demand, publishes successful pages independently, and retains cursors only inside App Server. Discovery starts when Task Navigation gains a subscriber, repeats every five minutes while demand remains, and also responds to `taskNavigation/refresh` and `taskNavigation/loadMore`. Opening an existing Task may request a non-blocking observation for its bound Native Session, but that observation never decides or delays the current attachment recovery. Sending a message does not initiate discovery.
...
An Open Task or unarchived Native Session may be pinned through its dedicated App Server metadata mutation. Pinned entries stay in their Project group and sort ahead of every unpinned entry; flat Navigation uses the same pinned-first ordering without adding a separate group. Within pinned and unpinned tiers, active-first and activity ordering are unchanged. Pins count toward the ordinary presentation limit, so a selected unpinned Task is not injected ahead of a full pinned window. Pinning is durable OpenAIDE metadata, does not count as activity, and waits for App Server confirmation before Navigation reorders. Archiving clears the pin permanently; Restore does not revive it. Prepared and Archived entries cannot be pinned.
...
`nativeSession/setTitle` and `nativeSession/setPinned` mutate only OpenAIDE-owned catalog presentation metadata. Agent discovery updates cannot overwrite a User title or pin. A normal history-backed adoption carries both values into the resulting Task, preserving the User title source; it still requires successful `session/load` and never manufactures a Task without Agent history. Native Session rows use the same Details, Rename, Pin, Fork, and Archive action vocabulary as adopted Tasks, while selecting the row itself remains the Open action.

`nativeSession/fork` is an experimental, capability-gated lifecycle operation over either an Open Task's bound Native Session or an unarchived Native Session catalog entry. Frontend identifies the source through a tagged Task or Native Session reference; App Server alone resolves the source Native Session, primary workspace, and effective MCP configuration. Fork is available only when the Agent advertises ACP Fork, Load, and Close support and the source has no additional workspace roots. Archived sources must be restored first. OpenAIDE does not impose an idle boundary: the Agent owns whether the selected active or inactive session can be forked and may reject the request.

One accepted Fork action dispatches ACP `session/fork` once and is never replayed automatically after an unknown transport outcome. A successful response's returned Native Session id is authoritative even when `session/list` never reports the fork. App Server durably seeds an unowned Native Session catalog entry in the source Project and workspace, captures a local fallback label such as **Fork of Fix login** without creating semantic parent linkage, publishes the Project replacement, and immediately calls `session/close` to release the unused active fork. Close cleanup failure is a warning and does not turn the created fork into a failed result. When Session List is available, later discovery may replace the local fallback with Agent-owned metadata.

Fork creates no OpenAIDE Task, copies no Chat or Tool history, and does not adopt or navigate to the returned Native Session. Frontend stays on the source surface and shows transient row-local pending, created, definite-failure, or unknown-outcome state. The action appears without confirmation only in active Task and Native Session row menus. OpenAIDE supplies the source `cwd` and currently resolved MCP configuration without a picker; the Agent owns validation and the copied conversation context.
...
`task/archive` changes an idle Open Task to Archived. A Task with active Agent work or a pending request must be stopped or resolved first. When the Task Message Queue is nonempty, Archive first requires destructive confirmation that names the queued-item count; confirmation atomically clears the queue and its App Server-managed attachments before archiving, while cancellation changes neither. Archiving clears process-local Agent catalogs and controls. `task/restore` changes an Archived Task to Open but does not load or resume its Native Session.

`task/archiveOlder` is a Project-scoped cleanup mutation keyed by one Open Task or unarchived Native Session cutoff row. It matches Open Tasks and unarchived, unowned Native Sessions whose last activity is strictly older than the cutoff; the cutoff itself and other Projects are excluded. Preview and apply use the same authoritative eligibility rules. Pinned entries plus active or waiting, queued, pending-request, and actively subscribed Tasks are reported as protected instead of being changed. Apply returns the exact archived identities and any Tasks protected by a concurrent state change, then publishes the affected Project membership for both Navigation sections.

Archived Tasks are saved, read-only history. `task/open` returns their stored state without Agent session load, resume, or history synchronization. Send, cancel, configuration mutation, and other ordinary Agent-interactive operations reject Archived Tasks until Restore. Explicit confirmed Delete is the exception. Late Agent events cannot reclassify an Archived Task or restore live controls. The sidebar presents Archive as a secondary destination from Tasks, visually identifies the read-only context, and provides Restore plus capability-gated Delete.

## Delete And Missing-Session Reconciliation

Status: accepted design; implementation tracked in [the session deletion plan](plans/session-deletion.md) and [ADR 0059](adr/0059-agent-confirmed-deletion-and-history-reconciliation.md).

Delete is a single-entry action for Open and Archived Tasks and unadopted Native Sessions in either Navigation section. All app shells expose it with destructive red styling only when the Agent advertises session deletion support. Archive and Delete each show an explanatory confirmation: OpenAIDE Archive is local and reversible; Delete removes the entry and saved history from OpenAIDE and asks the Agent to remove its session from history. Agent data may be retained, including through Codex thread archiving. The confirmation names any queued messages to be discarded. Bulk Delete is outside this feature.

Active Agent work or pending permission requests do not require a separate Stop before Delete. They require a second confirmation, after which the Agent owns whether and how active-session deletion succeeds. App Server rechecks activity before dispatch, including work started by another client after the initial confirmation, and requests the additional confirmation if needed. One App Server-owned session lifecycle gate blocks new work while deletion is unresolved. Agent rejection or an unknown transport outcome retains the entry and history, reports the failure or uncertainty, and offers explicit retry rather than claiming success or silently retrying the mutation.

After Agent success, App Server durably removes local visibility and uses existing local tombstone/purge cleanup for Task metadata, Chat, inline images, Tool artifacts, queued messages, and that Task's Composer History. It releases ephemeral attachment handles but does not delete referenced filesystem paths, project files, or worktrees, and preserves existing temporary-upload lifetimes. Unadopted session deletion does not require creating a Task. Authoritative removal reaches every client; a client viewing the removed entry moves to New Task.

General missing-session handling removes the same local entry and saved history, including Archived Tasks, when authoritative evidence establishes removal from Agent history. A definitive missing response from an ordinary session open qualifies; transient errors, authentication failures, and unclassified Agent errors do not. Absence from `session/list` qualifies only when an existing refresh successfully scans from the first page to the Agent's actual terminal cursor for the same Agent/workspace/filter scope. Bounded scans, failures, repeated cursors, and no-progress stops do not establish absence. Completion must be represented separately from those stop conditions, and reconciliation must preserve newer observations and exclude identities outside the scanned scope.

Reconciliation reuses existing discovery demand and adds no separate per-Task checks when browsing Archive or extra enumeration requests. Cleanup can wait until a bounded refresh naturally completes. There is no special durable remote-deletion recovery ledger; after a crash between Agent success and local removal, ordinary reconciliation handles the stale entry when authoritative evidence becomes available. Existing local tombstone/purge recovery handles interrupted physical cleanup once local removal is committed.
...
The sweep runs once when the reusable App Server starts and every six hours afterward. A Task migrated from a version without usage markers receives one measured seven-day grace period because prior client opens cannot be reconstructed safely. Expiration removes only that state root's local Task metadata, Chat, and Tool artifacts. It does not call Agent session deletion and does not remove or modify a Managed or External Worktree. Before removing a Task, App Server durably hides its Agent-owned Native Session identity from current and future discovery, so local cleanup cannot resurrect it as an unadopted row. App Server publishes Task removal before reclaiming its directory, and interrupted physical cleanup remains hidden and retryable on startup.
...
The first explicit use of built-in Codex installs OpenAIDE's exact version-locked Codex integration in durable per-user storage. App Server owns one shared ephemeral Installing Agent Status across Task preparation, Task Navigation, and Agent Settings; those surfaces render **Installing the Codex integration…** and may add **This can take a minute.** The install is coalesced across callers and processes, atomically published only after validation, and reused on later launches. Passive Native Session Discovery skips an unprovisioned managed integration instead of causing a download. After a successful explicit install, App Server starts Codex and reconciles any waiting preparation or discovery automatically, but never replays an accepted prompt or other work-producing operation.
...
When Node.js tools are available but Codex integration download, process launch, or ACP initialization fails, New Task says **Couldn't start Codex** and offers **Try again** and **Open Agent settings**. It does not guess that network access is the cause or expose raw technical details; diagnostics retain the classified failure for support.
...
Task Navigation keeps saved Tasks and its ordinary empty state authoritative when optional Native Session Discovery fails. It renders a separate, non-blocking notice that names the affected Agent. Authentication blockers say **Codex needs sign-in** (using that Agent's label) and offer **Sign in** and **Disable Codex**. **Sign in** opens that Agent's Settings detail without changing the retained New Task context; **Disable Codex** turns the Agent off so discovery stops asking for it. Other discovery failures keep exactly one cause-specific action: **Set up Codex** or **Try again**. Task Navigation never duplicates credential controls and never presents Native Session Discovery failure as an inability to load saved OpenAIDE Tasks.
...
A Send accepted while the Task is `working` is a steering message. App Server durably appends its text, Images, and file attachments to Chat and returns authoritative Task state, then asks `NativeSessionService.steer` to forward that accepted content to the same Native Session. The workflow does not wait synchronously for delivery or Agent completion.

When ACP `initialize._meta.steering.supported` is `true`, the Native Session runtime serializes delivery through `_session/steering`. An `injected` acknowledgment confirms acceptance for delivery, not that the model has consumed the message or finished work; the original prompt response continues to own Task completion. App Server keeps an admitted steering delivery within that ownership boundary even when the original prompt response arrives before its acknowledgment. The request opts into `_meta.steering.idleBehavior: "promptRequired"`: if the Agent explicitly reports that no running turn accepted the content, App Server sends it once through a tracked `session/prompt` and transfers completion ownership to that continuation. It does not replay an ambiguous or failed delivery. A failure is visible in Chat while the original prompt retains its lifecycle. The managed Codex adapter includes the same host-owned idle fallback supported by the pinned Claude adapter.

Agents without that advertisement retain the existing additional-`session/prompt` behavior below. A method-not-found response also selects that fallback and disables the extension for the live attachment. Other errors, timeouts, malformed replies, and detached `startedNewTurn` outcomes never trigger automatic replay.

In the prompt fallback, the primary prompt and every steering prompt accepted during its active Task turn form one current prompt set. The first current prompt response with `end_turn`, whether it belongs to the primary prompt or a steering prompt, ends that Task turn and publishes idle state. Prompt settlement and Send admission share one per-Task ordering boundary: a Send ordered after settlement starts a new Task turn even when its client has not yet observed idle, while fallback steering already accepted before settlement remains part of the ended prompt set. OpenAIDE then retires the remaining response waiters from lifecycle ownership; their eventual responses are stale and cannot finish, revive, block, or otherwise mutate the ended turn. Session updates remain owned by the session-lifetime update consumer and continue to be projected in arrival order.

### Selected-text Quotes

A **Quote** is ordinary editable User text, not a relationship to its source. Selecting visible text wholly inside one rendered User or Agent text body can show a small, viewport-clamped **Quote** action after pointer, touch, or keyboard selection settles. The selection must contain at least three non-whitespace Unicode grapheme clusters after removing only outer whitespace. A selection that crosses message boundaries or separate text bodies, includes row chrome, or originates in Tool, Thought, Permission, Question, Plan, or Live Activity presentation has no Quote action. Agent text may be quoted while streaming; Quote captures only the currently visible selected text.

Quote preserves selected internal spacing and line breaks. Every resulting line is prefixed with `> ` (or `>` for a selected blank line). It appends those quoted lines to the end of the current plain-text Composer draft, ignoring any prior Composer caret or selection and preserving existing draft text. When the preceding draft text is not already on a new line, Quote inserts one newline before it; Quote itself ends with one trailing newline, so the caret lands immediately on the following line. Activating Quote dismisses the action and focuses Composer there.

Quote is available whenever the active Task has an editable Composer, including while Send is temporarily unavailable. Archive and setup or authentication states that replace Composer hide it. The action dismisses when the selection collapses, Escape is pressed, the user interacts elsewhere, the route or source content changes, or Quote is activated. It does not alter ordinary selection or Copy behavior. Quote creates no source metadata, protocol field, persistence behavior, navigation affordance, or Agent prompt wrapper; existing Send, Queue, Composer History, persistence, and prompt paths handle the resulting draft text unchanged.

### Task Message Queue

The Composer exposes **Queue** while a Task is `starting`, `working`, `waiting`, or `stopping`. Queue accepts the current message as a durable Queued Message without adding a User message to Chat or invoking ACP. An idle Queue Pause does not expose Queue, so new items cannot be appended until explicit work begins again. If the active turn finishes before App Server serializes an already-invoked Queue mutation, App Server still accepts the item and immediately starts it as the next turn.

One Task Message Queue contains at most 20 items and at most 512 MiB of App Server-managed queued attachment data. It stores message content only, not Agent Configuration Options, isolation, model, or other Task configuration. Delivery uses the Task's current validated configuration and pauses before User-message acceptance when that configuration is unavailable.

App Server owns stable Queued Message identities, exact tail-append order, a queue revision, delivery eligibility, Queue Pause and its typed reason, and attachment lifetime. The bounded protocol-safe projection is part of `TaskSnapshot` and the contiguous Task revision stream; raw paths and retained attachment bytes remain App Server-only. Edit in Composer, move, remove, Resume Queue, and manual queued-message Send mutations name the revision they observed. A stale mutation is rejected with authoritative Task state instead of silently overwriting another client. Tail append remains safe under the per-Task serializer. Frontend issues every queue mutation once, uses its client mutation identity only to correlate authoritative acceptance, and never automatically replays an unknown outcome.

Automatic delivery is serialized with every queue mutation through the same per-Task acceptance boundary. Only an ACP `end_turn` that normally settles the current prompt set makes the head eligible. App Server atomically removes that head, appends its normal durable User message, records Composer History when text is present, and accepts one new Task turn before attempting ACP delivery. No client or recovery path can observe the same item as both queued and accepted Chat. `max_tokens`, `max_turn_requests`, `refusal`, `cancelled`, transport or protocol failure, forced termination, and unsuccessful prompt preparation leave remaining items intact and enter Queue Pause after the Task becomes idle. Every App Server restart also pauses a nonempty queue; recovery never advances it automatically.

A failure before the atomic User-message acceptance keeps the undeliverable head in the queue, preserves its position and attachments, records a typed visible reason, and pauses. A failure after acceptance belongs to the normal Task turn: the consumed item remains in Chat, is never requeued or replayed, and only the remaining queue pauses. **Resume queue** clears Queue Pause and starts the current head when the Task is idle.

Queued Messages also have an explicit manual Send action. It atomically consumes any selected item while preserving the relative order of the remainder, then follows ordinary Send semantics: it steers when the Task is `working` and starts a new turn when the Task is idle. Automatic delivery alone is head-first and always starts a separate turn. Ordinary Composer Send, manual queued-message Send, or Resume Queue from Queue Pause clears the pause before Agent work begins; Queue Pause therefore exists only while the Task is idle.

The queue renders in a bounded-height, collapsible panel above the Composer whenever it is nonempty. The panel shows complete delivery order, safe attachment metadata, pause or validation state, and per-row **Edit in Composer**, **Send now**, and **Remove from queue** controls. The action group is always visible on touch surfaces and appears immediately on row hover or keyboard focus when hover is available. When the current Agent Plan and Queue appear together, both disclosures place content above a bottom title row and use the same separation rhythm between each other and the Composer. Opening either disclosure transfers a following Chat viewport to Reading ownership before the panel changes height, so local inspection does not move Chat; reaching the latest Chat position through reader input restores Follow ownership. A queued-message row supports pointer dragging across its full non-action surface, while touch dragging uses the visible grip so vertical row swipes retain native Queue scrolling. An insertion marker previews the exact before-or-after drop position. Drag drops animate affected rows into their new positions.

**Edit in Composer** is available only while the ordinary Composer contains no text or attachments. App Server atomically removes the selected Queued Message at the observed queue revision and returns its text and attachments as new Composer-ready content. While confirmation is pending, the selected row remains visible with a pending indicator; after acceptance it collapses and Composer receives focus. A stale or failed mutation leaves both Queue and Composer unchanged. There is no separate edit mode, retained original, Save, Cancel, or delivery guard: after extraction, ordinary **Send** sends now and **Queue** appends the edited draft at the tail. The remaining queue continues normally. The extracted content has the same client-local lifetime as any ordinary draft, including possible loss on client reload or crash.
...
A non-empty incomplete Agent Plan remains durable across prompt completion, Stop, refusal, failure, reconnect, history synchronization, and App Server restart. Frontend pins it in one collapsible panel above the Composer. A new Plan is collapsed by default. Frontend-owned collapse state is retained per Task across updates, Task navigation, and page reloads in the current browser tab. Clearing, completing, or user-closing a Plan clears its retained disclosure state, so a later Plan starts collapsed again. When collapsed, the panel retains the current `in_progress` entry, or the first pending entry when none is explicitly in progress. High and low priority use quiet text; medium has no priority decoration.

Hiding a Plan drawer (header chip or **Hide Plan**) only conceals the panel; the Plan stays current. **Close Plan** lives in the Plan overflow menu even while the Task is active. App Server atomically removes the current Plan and appends one collapsed **Plan closed by user** row to Chat, preserving every entry's last reported status. The action is disabled while its mutation is pending. A later non-empty Agent Plan snapshot starts a new current Plan, including when the Agent sends it after the user closes it during the same active turn. A stale close request that no longer has a current Plan fails without adding history. The close and its Chat row survive ordinary App Server restart and cached Task reopen. ACP has no client-originated Plan-close operation, so a later authoritative `session/load` still replaces Chat with exactly the Agent replay as specified under Native Session Synchronization; a replayed non-empty Plan may therefore become current again.
...
An explicit fenced `mermaid` block in an ordinary completed Agent message may render as a static Diagram when it nears the viewport. The durable Chat value remains the Mermaid source, and live Agent text, Thoughts, Tool details, Skills, Settings, and User messages retain source presentation. While rendering, the complete source remains visible with an explicit progress status. Frontend renders through a packaged network-disabled sandbox and displays only the resulting inert image; the parent App Shell never inserts generated SVG markup. Invalid, oversized, timed-out, or unavailable renders preserve the complete source, with an explicit retry only for transient failures. A successful Diagram initially fits its available canvas, supports bounded zoom and pan both inline and expanded, retains source-copy and source-view actions, and regenerates when the App Shell theme changes. This shared behavior is identical across Web and VS Code App Shells.
...
Web/Desktop also expose **Project files** on the Task Page, independently of Agent output. Its Files, Search, and Changes navigator shares the existing read-only file/tab lifecycle. Opening this destination keeps Chat and the Composer draft mounted; Back to conversation restores them. Wide screens retain navigator and reader; narrow screens navigate list → reader → list. Source/diff text or gutter line selections expose Comment beside the completed selection and insert an editable File Quote into Composer without sending. Archived Tasks remain readable with no Comment action. This accepted expansion is specified in [the Project file access plan](plans/project-file-access.md) and ADRs 0033, 0035, 0038, and 0039.
...
Codex subagent metadata is interpreted only at the Codex integration boundary. Without bilaterally negotiated native Subagent Sessions, each recognized ACP Tool call remains a distinct transcript row keyed by `toolCallId`; start and interaction calls for the same subagent are not merged into an invented lifecycle. The compact row uses the Agent-provided Tool title. Its disclosure shows the subagent path, thread id, and activity exactly once, preferring `_meta.codex.subagent` and falling back to equivalent raw-input fields. Later updates to the same Tool replace that row's status without creating generic Tool noise. Recognized Codex `wait` collaboration calls use the product title `Wait for subagents` because observed ACP payloads may contain no receiver identity or agent state; they must not imply that a particular subagent completed. The generic Tool projection remains the fallback for malformed or unknown metadata.

### Native Subagent histories

When both sides negotiate canonical ACP Subagent Sessions, App Server represents every announced child session as a Task-owned Subagent. It assigns a stable product Subagent id to each newly announced lifecycle generation and keeps the Agent's child session id private at the integration seam. Replaying the same announced generation reconciles that identity; a later reopened worker is a new Subagent rather than a revival of a terminal generation. Subagents may nest, and the catalog preserves sibling spawn order without claiming a global order between independent session streams.

Main Agent Chat receives one durable clickable lifecycle row at the spawn position. That row updates to the child's current lifecycle but never duplicates child messages, thoughts, plans, Tools, permission outcomes, or questions. Legacy Tool rows and later native lifecycle rows may coexist after an upgrade, but one replayed lifecycle is never projected through both representations. Passive Task open preserves the saved representation. Only an explicit successful `session/load` may replace it with an authoritative native reconstruction, and the replacement commits only after the complete replay validates.

Task snapshots expose only a lightweight Subagent overview. A dedicated paged and subscribed catalog owns the complete nested hierarchy, while a separate paged and subscribed history scope owns the selected Main Agent or Subagent history. Each Subagent history has its own durable snapshot and journal under the Task directory. Missing or corrupt child history marks only that history unavailable; it never quarantines the Task or Main Agent Chat. A Subagent may be `waiting for activity`, `running`, `completed`, `failed`, `cancelled`, or `disconnected`; `disconnected` means the terminal outcome is unknown and is not presented as failure.

When the first native Subagent appears, Frontend adds a persistent Agent navigator above history. `Main Agent` is always a one-action return to Task Chat. The selector lists the complete spawn-ordered nested tree and permits direct switching between any children; keyboard navigation follows the same model. Selecting a Subagent replaces Task Chat at every viewport width and never opens a second split layout. New children and terminal changes update the navigator and accessible status announcement but never auto-switch or auto-return. Selection, branch collapse, and per-client unseen activity are ephemeral and reset to Main Agent when the Task unmounts or the page reloads.

Selected Subagent History uses the same message, Thought, Plan, Tool, and outcome presentation as Chat, but it is observational and has no Composer unless that specific child advertises a future standardized input capability. A child Plan is scoped to that history and never replaces the Main Agent Plan. The selected header shows concise lifecycle state and an optional compact Details disclosure for safe normalized Agent-specific metadata; raw ACP `_meta` never reaches Frontend. **Quote in Main Agent Chat** returns to Main Agent, inserts an ordinary editable quote into Composer, and creates no durable source relationship.

Permission and Question requests remain Task-wide workflow state. One actionable surface stays visible above whichever history is selected and attributes the request to its Subagent and Tool; history records activity and outcome without duplicating the response control. Child failure does not fail the Task, whose turn status remains Main Agent-owned, though a pending child request may make the Task `waiting`. Targeted Stop is absent unless that child positively advertises cancel or close; stopping one child cascades only to its descendants. Invalid native traffic is rejected with redacted diagnostics while Main Agent history stays available. OpenAIDE never downgrades an already negotiated live session; an internal capability-advertisement override affects later Agent connections only.

For high-confidence, read-only shell forms, App Server may persist ordered semantic presentation actions (`skill`, `read`, `list`, or `search`) on an `execute` Tool. Search actions preserve separate query, scope, and content-versus-path target facts instead of flattening them into display text. One homogeneous action retains its specific compact title and icon. A command list proven to contain only Read and Search actions renders both ordered clauses with the file-search icon; a proven Skill instruction read retains its Skill identity after that safety decision. This Read-family plus Search sequence is the only supported mixed combination. Pipelines are supported only when every stage and the full data-flow relationship are proven: a supported Search may feed a stdin-only `head` line limiter, `rg --files` may feed a supported `rg` filter to mean a file-name search, and exact `nl -ba FILE | sed -n PRINT_SCRIPT` compositions may select one or more semicolon-separated numeric print ranges as a numbered Read. Any other pipeline falls back to Execute. Compact titles show the shortest unique search-scope suffixes, present the exact scope `.` as `workspace`, and summarize extra read subjects as `A, B, and N more`. A visible arrow separates ordered actions. Long Search patterns use a bounded local ellipsis with a minimum `12ch` text window; if the complete title still does not fit, one title-level ellipsis truncates the row. Its hover tooltip retains the full safe relative scopes, complete query, and every subject in natural prose. Presentation changes only compact title, icon, and grouped summary. The Tool remains `execute`, and expanding it always preserves the complete execute command, output, status, permission history, and terminal details. Unknown syntax, any potentially effectful form, and every unsupported mixed combination receive no presentation because an incorrect semantic claim is worse than the Execute fallback.
...
App Server preserves every ACP `session/prompt` response and `stopReason`. With the steering extension, the owning prompt response changes Task status from `working` to idle after admitted deliveries resolve; an explicit `promptRequired` continuation replaces that owner. With the prompt fallback, the first `end_turn` response from any prompt in the current prompt set changes Task status to idle. Neither path finalizes Chat messages, Tool state, or the Native Session update consumer. Other stop reasons retain their prompt-specific handling below; a fallback steering response with another stop reason does not settle the Task turn.
...
Unexpected runtime and Agent errors use stable product language in Chat. Provider payloads, internal identifiers, and diagnostic error strings remain in App Server diagnostics and are never persisted into user-visible interruption messages.
...
App Server releases an inactive Native Session after 30 minutes without session activity when the Agent advertises ACP `sessionCapabilities.close`. A newly created Native Session is not idle-released until its first prompt completes successfully because an Agent may not make an empty session durable; explicit Prepared Task disposal and App Server shutdown still close it. Loaded and resumed sessions are already eligible for idle release. Activity is measured by the Native Session worker, not Task Page visibility or subscription count. Session setup, outbound session operations, and inbound `session/update` notifications restart the deadline. A prompt, Configuration Option mutation, permission, question, or other in-flight session operation suspends expiration until that operation settles.
...
The VS Code Extension may present a Task Attention Event through VS Code's workbench Notification Center while its extension host is active. An event is eligible when its Task was not the active Task editor at the event occurrence time, including while another Task, source file, or workbench surface was active. Losing VS Code window focus does not make the active Task eligible. Its first Task-navigation snapshot establishes a baseline and never turns existing attention into a notification backlog. Extension-global delivery receipts suppress replay across restart, and VS Code's own per-extension notification filter and Do Not Disturb controls provide local enablement. The notification contains only the short product reason and a single-line, compact form of the Task title; selecting its **Open Task** action routes to the Task without changing its complete title. VS Code extension notifications stay inside the VS Code workbench.

App Server owns Task Attention meaning, identity, persistence, ordering, and clearing through product mutations. The Web App shell owns local opt-in, browser permission, focus observation, cross-tab coordination, delivery receipts, OS notification presentation, replacement, closing, and Task routing. The VS Code shell owns focused-Task observation, extension-global delivery receipts, workbench notification presentation, and Task routing. Shared Frontend may carry the authoritative event to a shell through a narrow presentation seam, but it does not infer attention policy or call notification APIs directly. The existing client-scoped `shell/showNotification` request remains available for explicit App Server-to-shell messages and is not the Task Attention lifecycle.
...
Opening an existing Task returns stored Task and Chat state immediately and marks cached Native Session controls as recovering. A Task without a deferred reload requirement recovers its known binding directly through `session/resume`; `session/list` never gates this resume. When resume is unsupported, recovery may use `session/load` as the one restore operation for that attachment. Send never initiates catalog or Native Session synchronization.

App Server persists a deferred reload requirement only when a complete Native Session Catalog observation carries valid, comparable activity that is newer than the Task's local replayed Chat activity by the accepted tolerance. The timestamp is a possible-external-change hint, not proof of new Chat. Listing omission, failure, invalid activity, or equal-or-older activity changes nothing. A later observation may advance the requirement monotonically, but it never closes, resumes, or loads an already live attachment.
...
App Server owns a durable bounded Composer History projection because Native Session history replacement cannot preserve trustworthy OpenAIDE-send provenance or original cross-Task message timestamps. The projection records text only after ordinary or queued-message Send durably accepts a User message, together with the originating Task, Project, stable entry identity, and acceptance order. Initial prompts, follow-ups, steering messages, and automatically or manually delivered Queued Messages qualify at delivery acceptance. Failed or uncertain Sends, unsent drafts, not-yet-delivered or removed Queued Messages, and attachment-only messages do not. Text sent with attachments contributes only its text. Native Session user messages loaded from another client do not enter Composer History.

Duplicate detection compares the exact text after normal Send normalization. Accepting the same text again moves that value to the newest position rather than creating another recall entry; case and meaningful whitespace differences remain distinct. Archive does not remove entries. Explicit Task deletion and authoritative missing-session cleanup remove that Task's Composer History contribution; identical text independently contributed by another Task remains eligible. The projection retains enough bounded entries to return 50 unique values for either supported scope.
...
- On Send, Frontend encodes the current Images inline in `task/send`. It first uses the normal reliable-session request. If an intermediary rejects that request with HTTP 413 or an HTML HTTP 403, the transport retries the exact same sequenced request as base64-encoded chunks whose decoded payloads are no larger than 512 KiB. App Server reassembles that reliable-session frame in bounded, expiring memory and then performs the normal atomic Send; no Image is written to the filesystem. App Server validates supported MIME types, encoding, individual and aggregate limits, message shape, and Agent Image capability in the same acceptance transaction.
...
- The Web App first streams each selected general file through one authenticated same-origin request directly into a private Task directory under the App Server's `openaide/uploads` OS temp area. The file uses a safe version of the original basename so Agent-facing file references remain recognizable; an atomic numeric suffix before the extension resolves a same-name collision. If that request receives HTTP 413, receives an HTML HTTP 403 from a request-size gateway, or fails at the transport boundary, the Web App automatically restarts that file from byte zero through an authenticated upload session using sequential binary chunks no larger than 512 KiB. App Server scopes the session to the initialized client, requires exact offsets and stable Task/file metadata, and creates a sendable attachment handle only after the declared byte count is complete. It runs at most two general-file uploads concurrently, renders per-file progress and cancellation, and blocks Send until every selected upload settles. A failed upload remains visible with a concise inline error, can be dismissed, and can be retried without reselecting while the browser still retains the selected file; after chunk fallback has begun, Retry starts directly in chunk mode. Known partial/failed files and chunk sessions are deleted or expired best-effort. Completed files remain under OS temp lifetime management. A sent-file click streams the still-available file back through an authenticated Task-and-message-scoped download route.
...
- On Queue acceptance, App Server promotes current attachment resources into durable queue ownership in the same atomic mutation. Edit in Composer performs the reverse transfer while atomically removing the item. Images and embedded snapshots are captured; live file references retain their resolver semantics and are revalidated before delivery or extraction.
- Removing or replacing a Queued Message releases only the managed attachment data no longer owned by another durable message. A missing, stale, or newly unauthorized live file reference pauses before User-message acceptance with the original item intact.
...
13. Product-client inactivity expires client-scoped state after reconnect grace, and expiry of the final App Shell client begins graceful App Server shutdown; a reconnect during draining aborts shutdown, while a changed process endpoint recovers through the same logical-session baseline barrier without mutation replay.
14. Task Message Queue mutations and automatic delivery share one durable per-Task ordering; one Queued Message can never be both pending and accepted Chat, and an accepted or uncertain ACP prompt is never requeued or replayed.
15. Only normal `end_turn` advances the queue automatically. Every other terminal outcome and every App Server restart preserves pending items behind an idle Queue Pause until explicit work clears it.
