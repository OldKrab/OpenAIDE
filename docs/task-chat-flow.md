...
Only the Native Session Catalog module calls `session/list`. It discovers all enabled Agents for every visible Project root and available worktree, coalesces overlapping refresh demand, publishes successful pages independently, and retains cursors only inside App Server. Discovery starts when Task Navigation gains a subscriber, repeats every five minutes while demand remains, and also responds to `taskNavigation/refresh` and `taskNavigation/loadMore`. Opening an existing Task requests one targeted fresh observation for its bound Native Session before Native Session recovery chooses `session/resume` or `session/load`; this background observation never delays the stored Task response. Sending a message does not initiate discovery.
...
The current Plan panel exposes a compact **Close Plan** action even while the Task is active. App Server atomically removes the current Plan and appends one collapsed **Plan closed by user** row to Chat, preserving every entry's last reported status. The action is disabled while its mutation is pending. A later non-empty Agent Plan snapshot starts a new current Plan, including when the Agent sends it after the user close during the same active turn. A stale close request that no longer has a current Plan fails without adding history. The close and its Chat row survive ordinary App Server restart and cached Task reopen. ACP has no client-originated Plan-close operation, so a later authoritative `session/load` still replaces Chat with exactly the Agent replay as specified under Native Session Synchronization; a replayed non-empty Plan may therefore become current again.
...
Opening an existing Task returns stored Task and Chat state immediately, marks cached Native Session controls as recovering, and refreshes that bound session's catalog entry in the background. A successful fresh observation chooses exactly one recovery operation: `session/load` when Native activity proves the local projection stale, otherwise `session/resume`. Failed listing or a complete listing that omits the stored identity falls back to `session/resume`; unsupported resume falls back to `session/load`. Send never initiates catalog or Native Session synchronization.

After a complete later catalog observation, any newer Native activity triggers whole-session synchronization for an idle Task with an active Task-scope subscriber. App Server cannot distinguish a history change from another external session change using the catalog timestamp, so it replaces the complete Native Session projection rather than claiming to update only Chat.

Replacement shares one per-Task lifecycle gate with prompts, steering, Configuration Option mutations, commands, cancellation, Archive, and other session operations. If a user operation wins that gate, the passive replacement is abandoned until the next Task open. If replacement wins, later operations wait, and further passive observations coalesce without replacing its visible synchronization generation. App Server marks cached controls recovering, detaches the current attachment, sends ACP `session/close` when supported, and creates a new attachment through `session/load`. The Agent-returned Chat replay, Configuration Options, commands, title, capabilities, and other session metadata are authoritative and become visible in one Task update. Closing the Task Page does not cancel a replacement already in progress.

While replacement runs, Frontend keeps stored Chat and last-known controls visible, locks session controls, shows **Reloading session**, and disables Send. A successful load briefly shows `History updated` when Chat was replaced. Failure follows ordinary load-recovery behavior: it keeps existing Chat, appends `History update failed` Live Activity, marks cached controls stale, ends syncing, and enables explicit recovery.
