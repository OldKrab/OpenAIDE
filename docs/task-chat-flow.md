...
A non-empty incomplete Agent Plan remains durable across prompt completion, Stop, refusal, failure, reconnect, history synchronization, and App Server restart. Frontend pins it in one collapsible panel above the Composer. A new Plan is collapsed by default. Frontend-owned collapse state is retained per Task across updates, Task navigation, and page reloads in the current browser tab. Clearing or completing a Plan clears its retained disclosure state, so a later Plan starts collapsed again. When collapsed, the panel retains the current `in_progress` entry, or the first pending entry when none is explicitly in progress. High and low priority use quiet text; medium has no priority decoration.
