...
Codex subagent metadata is interpreted only at the Codex integration boundary. Each recognized ACP Tool call remains a distinct transcript row keyed by `toolCallId`; start and interaction calls for the same subagent are not merged into an invented lifecycle. The compact row uses the Agent-provided Tool title. Its disclosure shows the subagent path, thread id, and activity exactly once, preferring `_meta.codex.subagent` and falling back to equivalent raw-input fields. Later updates to the same Tool replace that row's status without creating generic Tool noise. Recognized Codex `wait` collaboration calls use the product title `Wait for subagents` because observed ACP payloads may contain no receiver identity or agent state; they must not imply that a particular subagent completed. The generic Tool projection remains the fallback for malformed or unknown metadata. Frontend exposes no thread navigation or orchestration controls unless a future Codex integration provides the corresponding capability.
