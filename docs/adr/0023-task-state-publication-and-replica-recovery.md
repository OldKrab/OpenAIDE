...
Product-client heartbeat expiry cleans up client-scoped product state but does not own App Server process lifetime. The native App Shell supervises the physical process endpoint independently, performs a new handoff after bounded health-probe failure, and supplies the changed endpoint to the existing logical `AppServerSession`. UI inactivity, suspension, and loss of scheduling therefore cannot terminate the App Server merely because the last product client expires.
