...
One per-Task lifecycle gate orders passive replacement against every user session operation. A user operation that acquires the gate first invalidates scheduled passive work; synchronization waits until the next Task open. Replacement that acquires it first marks catalogs recovering and holds later operations until the new attachment commits. Further catalog observations coalesce behind that replacement and cannot supersede its visible synchronization generation. Frontend keeps last-known controls visible but locked and labels the operation **Reloading session** until that generation publishes its terminal state.
