...
Each result reconciles Agent title metadata and advances a bound Task's activity only when the Agent timestamp is newer. After committing the complete listing observation, App Server may record a durable deferred reload requirement for a bound Task. Catalog discovery never replaces an active Task attachment.

## Native Session Synchronization

Opening an existing Task returns stored state immediately and recovers a known binding without first listing it. With no deferred reload requirement, recovery resumes that binding directly and attaches the update consumer; a resume result may replace current catalogs but leaves stored Chat unchanged. If resume is unsupported, recovery uses load as the only restore operation for that attachment. Catalog demand remains asynchronous observation and cannot block controls.

When a complete catalog observation carries an Agent timestamp that is present, comparable, and more than five seconds newer than `localHistoryUpdatedAt`, App Server records a deferred reload requirement. The fixed tolerance absorbs normal delay between App Server persisting an Agent update and the Agent persisting its session timestamp. Missing, invalid, equal, older, or no-more-than-five-seconds-newer timestamps create no requirement. The requirement survives App Server restart and advances monotonically with newer comparable observations.

The requirement is a possible-external-change indicator, not a proof of changed Chat. Catalog discovery never runs `session/load`, closes an attachment, or locks an editable Task. It publishes `historySync: reloadAvailable` so Frontend can present an inline Reload action while leaving Composer and Configuration Options usable.

An explicit `task/reloadNativeSession` mutation is the only way to load an already-resumed attachment. It acquires the Task lifecycle gate, publishes `historySync: syncing`, detaches and closes the attachment when supported, then loads once. A successful load atomically replaces Chat and Agent-owned metadata, clears only the captured-or-older requirement, publishes `updated`, and attaches the replacement consumer. A failure keeps stored Chat and the requirement, returns to `reloadAvailable`, and leaves explicit retry available. Delayed work from an older attachment cannot mutate a newer attachment.

If a Task is opened while it has a deferred requirement, App Server loads before exposing editable controls. It never resumes before that load, so every attachment recovery selects exactly one restore operation. Failed or unsupported load preserves the requirement and never falls through to resume. Send never checks or initiates synchronization. Live updates for App Server-owned Native Sessions continue through their Native Session update consumers.
