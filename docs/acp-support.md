...
| `session/set_config_option` | partial | session config options | Task Native Session config drives ordered select and boolean controls in Task snapshots. Each returned option list is the complete Agent-owned render state. Changes target the Task's existing Native Session, including during an active turn; OpenAIDE never creates a replacement session to apply configuration. |
...
| Boolean config option | supported | config option schema | Advertises the boolean client capability, preserves Agent order, renders compact text-first switches, and sends typed boolean values through `session/set_config_option`. Malformed or unknown option kinds are ignored. |
