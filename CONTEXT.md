...
OpenAIDE may locally archive an unowned catalog entry by Agent Identity plus Native Session identity. Archived entries remain Agent-owned, stay excluded from active discovery across later listing pages, and return only through OpenAIDE Restore. They never call an Agent deletion or loading method. Active discovery depth counts only visible Open Tasks and unarchived, unowned Native Sessions, so pages containing only archived or owned identities do not satisfy Load More demand.
...
The App Server-owned, local, reversible visibility state for one discovered, unowned Native Session. Identity is Agent Identity plus Native Session identity. It does not adopt, load, resume, delete, or otherwise mutate the Agent-owned session.
_Avoid_: Archived Task, ACP session deletion, Frontend hidden-session storage
