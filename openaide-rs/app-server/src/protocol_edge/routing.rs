...
impl RpcGateway {
    ...
    fn handle_inbound_inner(
        ...
        match method.as_str() {
            ...
            AGENT_LOGOUT => unreachable!("shared gateway routes logout outside the protocol lock"),
            AGENT_CANCEL_AUTHENTICATE => {
                self.handle_agent_cancel_authenticate(connection_id, id, params, meta, now)
            }
            ...
            NATIVE_SESSION_DELETE => {
                self.handle_native_session_delete(connection_id, id, params, meta, now)
            }
            NATIVE_SESSION_FORK => {
                self.handle_native_session_fork(connection_id, id, params, meta, now)
            }
            AGENT_CREATE_CUSTOM => {
            ...
            MCP_GET_SERVER_DETAILS => {
                self.handle_mcp_get_server_details(connection_id, id, params, meta)
            }
            MCP_CREATE_SERVER => self.handle_mcp_create_server(connection_id, id, params, meta),
            MCP_UPDATE_SERVER => self.handle_mcp_update_server(connection_id, id, params, meta),
            MCP_DELETE_SERVER => self.handle_mcp_delete_server(connection_id, id, params, meta),
            MCP_SET_SERVER_ENABLED => {
                self.handle_mcp_set_server_enabled(connection_id, id, params, meta)
            }
            SETTINGS_GET_SKILLS => self.handle_settings_get_skills(connection_id, id, params, meta),
            SETTINGS_GET_SKILL_DETAILS => {
                self.handle_settings_get_skill_details(connection_id, id, params, meta)
            }
            ...
            PROJECT_ADD => self.handle_project_add(connection_id, id, params, meta, now),
            PROJECT_RENAME => self.handle_project_rename(connection_id, id, params, meta, now),
            PROJECT_REMOVE => self.handle_project_remove(connection_id, id, params, meta, now),
            PROJECT_REFRESH => self.handle_project_refresh(connection_id, id, params, meta, now),
