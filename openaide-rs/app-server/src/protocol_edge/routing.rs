use openaide_app_server_protocol::methods::{
    ...
    MCP_SET_SERVER_ENABLED, MCP_UPDATE_SERVER, NATIVE_SESSION_ARCHIVE, NATIVE_SESSION_RESTORE,
    PENDING_REQUEST_RESOLVE, PROJECT_RECONNECT, PROJECT_REGISTER, PROJECT_REMOVE, PROJECT_RENAME,
    ...
    TASK_CHAT_PAGE, TASK_COMPOSER_HISTORY, TASK_LIST, TASK_MARK_READ, TASK_NAVIGATION_LOAD_MORE,
...
impl RpcGateway {
    pub fn handle_inbound(
        ...
        match method.as_str() {
            ...
            PROJECT_REGISTER => self.handle_project_register(connection_id, id, params, meta, now),
            PROJECT_RENAME => self.handle_project_rename(connection_id, id, params, meta, now),
            PROJECT_RECONNECT => {
                self.handle_project_reconnect(connection_id, id, params, meta, now)
            }
