...
use openaide_app_server_protocol::ids::{ClientInstanceId, ProjectId, StateRootId};
use openaide_app_server_protocol::snapshot::{
    AgentCollectionSnapshot, ProjectCollectionSnapshot, TaskStatus as ProtocolTaskStatus,
    TaskSummary,
};
...
fn first_event_after_subscribe_chains_from_subscription_cursor() {
    ...
    assert_eq!(publish.event.previous_cursor, subscribe.cursor);
...
fn task_navigation_publication_reads_snapshot_after_durable_write() {
    ...
    let AppServerEventPayload::TaskNavigationUpdated { navigation } = publish.event.payload else {
...
fn client_scoped_events_only_deliver_to_matching_client() {
    ...
    assert_eq!(publish.deliveries.len(), 1);
    assert_eq!(
        publish.deliveries[0].client_instance_id,
