...
The pinned `@openaide/codex-acp@1.2.0` adapter always supplies a provider when resuming or loading a session. Native Codex treats that as an explicit override and skips its persisted model/reasoning restoration. The adapter also rebuilds approval mode from its startup default. Native Codex 0.153.3 restores approval policy and reviewer but reloads the sandbox from current configuration, so forwarding its resume response alone can broaden an existing session's permissions.
